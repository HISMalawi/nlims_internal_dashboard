...
class HomeController < ApplicationController
    ...
    def search_eid_viral_data
        from = params["from"]
        to = params["to"]
        status = params["status"]
        tracking_number = params["tracking_number"]

        status = "none" if !tracking_number.blank?
        ...
        if lab_name.include?("Laboratory")
            ...
            if !from.blank? && !to.blank?                
                    if !status.blank? && status != "none"
                        ...
                        tst_status = TestType.find_by_sql("SELECT id AS tst_type FROM test_statuses WHERE name='#{status}'")
                        tst_status = tst_status[0]['tst_type']
                        ...
                        rs = Speciman.find_by_sql("SELECT tests.id AS test_id, specimen.id AS sp_id, test_statuses.name AS test_status, specimen.tracking_number, specimen.date_created, specimen.sending_facility FROM specimen
                                ...
                                OR substring(specimen.tracking_number,2,'#{lab_code.length}')='#{lab_code}')
                                AND (tests.test_status_id = '#{tst_status}' AND (DATE(specimen.date_created) >='#{from}' AND DATE(specimen.date_created) <= '#{to}'))
                                order by specimen.id DESC
                            ")


                    elsif !tracking_number.blank?                       
                        last_name = lab_name.gsub("Laboratory","")
                        rs = Speciman.find_by_sql("SELECT tests.id AS test_id, specimen.id AS sp_id, test_statuses.name AS test_status, specimen.tracking_number, specimen.date_created, specimen.sending_facility FROM specimen
                                INNER JOIN tests ON tests.specimen_id = specimen.id
                                INNER JOIN test_statuses ON test_statuses.id = tests.test_status_id
                                WHERE (tests.test_type_id = 71 AND specimen.tracking_number='#{tracking_number}') AND (specimen.target_lab ='#{lab_name}' OR  specimen.target_lab ='#{last_name}'
                                OR substring(specimen.tracking_number,2,'#{lab_code.length}')='#{lab_code}')                                
                                order by specimen.id DESC
                            ")
                    else
                        ...
                        rs = Speciman.find_by_sql("SELECT tests.id AS test_id, specimen.id AS sp_id, test_statuses.name AS test_status, specimen.tracking_number, specimen.date_created, specimen.sending_facility FROM specimen
                                ...
                                WHERE tests.test_type_id = 71 AND (specimen.target_lab ='#{lab_name}' OR  specimen.target_lab ='#{last_name}'
                                OR substring(specimen.tracking_number,2,'#{lab_code.length}')='#{lab_code}')
                                AND (DATE(specimen.date_created) >='#{from}' AND DATE(specimen.date_created) <= '#{to}')
                                order by specimen.id DESC
                            ")
                    end     
            elsif !tracking_number.blank?
            ...
            if !from.blank? && !to.blank?
                    ...
                    if !status.blank? && status != "none"
                        ...
                        rs = Speciman.find_by_sql("SELECT tests.id AS test_id, specimen.id AS sp_id, test_statuses.name AS test_status, specimen.tracking_number, specimen.date_created, specimen.sending_facility FROM specimen
                                INNER JOIN tests ON tests.specimen_id = specimen.id
                                INNER JOIN test_statuses ON test_statuses.id = tests.test_status_id
                                WHERE (tests.test_type_id = 71 AND specimen.tracking_number='#{tracking_number}') AND (specimen.sending_facility ='#{lab_name}'
                                OR substring(specimen.tracking_number,2,'#{lab_code.length}')='#{lab_code}')                                
                                order by specimen.id DESC
                            ")
                    else
