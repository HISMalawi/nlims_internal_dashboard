    ...
    module AcknowledgeResult
      class << self
        ...
        def count_all(start_date: R4hDashboard::Utils::General.start_date, end_date: R4hDashboard::Utils::General.end_date)
          ...
          sql = "SELECT COUNT(t.test_result_receipent_types) AS count, t.test_result_receipent_types as id FROM specimen sp INNER JOIN tests t ON t.specimen_id = sp.id INNER JOIN test_types tt ON tt.id = t.test_type_id
        ...
        def count_per_site(start_date: R4hDashboard::Utils::General.start_date, end_date: R4hDashboard::Utils::General.end_date)
          central_hospitals = R4hDashboard::Utils::General.central_hospitals
          sql = "SELECT COUNT(t.test_result_receipent_types) AS count, t.test_result_receipent_types as id, 
            sp.sending_facility AS facility, sp.district AS receiving_facility
        ...
        def drill_details(site_name: s_name, start_date: R4hDashboard::Utils::General.start_date, 
          end_date: R4hDashboard::Utils::General.end_date, acknowledgement_type: 2)
          central_hospitals = R4hDashboard::Utils::General.central_hospitals
          sql = "SELECT sp.tracking_number,t.test_result_receipent_types AS acknoledgemet_type, sp.sending_facility AS facility,
