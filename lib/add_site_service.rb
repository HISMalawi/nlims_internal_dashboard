module AddSiteService
    def self.seed_sites(sites)
        sites.each do |key, value|
            ...
            if site.nil?
                Site.create(name: key, ip_address: value[0], site_code: value[1], x: value[3], y: value[2], district: value[4], region: value[5], enabled: 1)
            else
                site.update(ip_address: value[0], site_code: value[1], enabled: 1)
            end
        end
    end
    ...
    def self.add_new_site(params)
        ...
        if site.nil?
            Site.create(name: params[:site_name].titleize, ip_address: params[:ip_address], site_code: params[:site_code].upcase, 
            ...
            site.update(ip_address: params[:ip_address],enabled: enable)
        else 
            Site.create(name: params[:site_name].titleize, ip_address: params[:ip_address], site_code: params[:site_code].upcase, 
                x: params[:longitude], y: params[:latitude], district: params[:district].capitalize, region: params[:region].capitalize, 
                description: params[:description].titleize,enabled: enable)     
